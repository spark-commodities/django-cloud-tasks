        ...
        def wrapper():
            ...
            while attempts_left > 1:
                try:
                    ...
                    error = e
                    logger.warning("Task scheduling failed. Retrying...")
...
class CloudTaskWrapper(object):
    ...
    def run(self, mock_request=None):
        ...
        :param mock_request: Task instances accept request argument that holds various
            attributes of the request coming from Cloud Tasks service. You can pass a
            mock request here that emulates that request. If not provided, default mock
...
def remote_task(queue, handler, task_handler_url=None, **headers):
    """
    Returns `RemoteCloudTask` instance. Can be used for scheduling tasks that are not
